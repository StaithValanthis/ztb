...
# ---------------------------------------------------------------------------
# ZTB-1845: Reduce-only zero-position guard
# ---------------------------------------------------------------------------


@patch("ztb.execution.executor.load_data")
@patch("ztb.execution.executor.BybitClient")
def test_reduce_only_skipped_when_exchange_position_zero_long(
    mock_bybit_cls: MagicMock,
    mock_load: MagicMock,
    sample_data: pd.DataFrame,
) -> None:
    """PnL long 2.0, signal flat, exchange position = 0 → order skipped, PnL zeroed."""
    mock_load.return_value = sample_data
    mock_client = MagicMock()
    mock_client.place_order.return_value = {"orderId": "oid"}
    mock_client.get_positions.return_value = []
...
@patch("ztb.execution.executor.load_data")
@patch("ztb.execution.executor.BybitClient")
def test_reduce_only_skipped_when_exchange_position_zero_short(
    mock_bybit_cls: MagicMock,
    mock_load: MagicMock,
    sample_data: pd.DataFrame,
) -> None:
    """PnL short -2.0, signal flat, exchange position = 0 → order skipped, PnL zeroed."""
    mock_load.return_value = sample_data
    mock_client = MagicMock()
    mock_client.place_order.return_value = {"orderId": "oid"}
    mock_client.get_positions.return_value = []
...
def test_reduce_only_proceeds_when_exchange_has_position_long(
    ...
    """PnL long 2.0, exchange long 2.0, signal flat → reduce-only proceeds normally."""
    mock_load.return_value = sample_data
    mock_client = MagicMock()
...
def test_reduce_only_proceeds_when_exchange_has_position_short(
    ...
    """PnL short -2.0, exchange short -2.0, signal flat → reduce-only proceeds normally."""
    mock_load.return_value = sample_data
    mock_client = MagicMock()
...
@patch("ztb.execution.executor.load_data")
@patch("ztb.execution.executor.BybitClient")
def test_reduce_only_guard_does_not_block_flip(
    mock_bybit_cls: MagicMock,
    mock_load: MagicMock,
    sample_data: pd.DataFrame,
) -> None:
    """Flip has reduce_only=False → guard does not apply, order proceeds."""
    mock_load.return_value = sample_data
    mock_client = MagicMock()
    mock_client.place_order.return_value = {"orderId": "oid_flip"}
    mock_client.get_positions.return_value = []
    ...
    exe = Executor(FlipToShortSignal(), config=config)
    exe._init_run()
    exe._init_store(":memory:")
    exe.client = mock_client
...
@patch("ztb.execution.executor.load_data")
@patch("ztb.execution.executor.BybitClient")
def test_reduce_only_guard_does_not_apply_in_dry_run(
    mock_bybit_cls: MagicMock,
    mock_load: MagicMock,
    sample_data: pd.DataFrame,
) -> None:
    """Dry run has no client → guard is skipped, reduce-only logic runs normally."""
    mock_load.return_value = sample_data
    config = ExecRunConfig(mode=Mode.DEMO, dry_run=True, risk_enabled=False)

    class FlatFromLongSignal:
        name = "flat_from_long_dry"
        symbols = ["BTCUSDT"]
        timeframe = "60"
        params: dict = {}
        warmup = 50
    ...
    assert result.get("order_placed") is False
    assert result.get("order_skipped") is not True
    assert abs(exe._pnl.position - 2.0) > 1e-8  # PnL unchanged (dry run uses its own PnL only)
