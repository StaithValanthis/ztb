...
- **Fix(engine):** Bar-0 accounting — unified `i==0`/`i>0` branches in `single_symbol_portfolio`, `multi_symbol_portfolio`, `risk_adjusted_signals`, and `risk.multi_symbol_portfolio` so all fills record trade entry with cash debit and costs (costs at bar 0 were silently absorbed into equity)
...
- **Tests:** 8 new bar-0 portfolio tests covering short entry, partial entry, enter-then-close, long-to-short flip, short-to-long flip, multi-symbol two-signal, equity continuity single + multi
- Coverage: executor 86%, bybit_client 47% (new method), portfolio.py 100%, total 92%
