...
- **Three-key merge:** CI green (test 3.11/3.13) + V&R PASS on SHA `${COMMIT_SHA}` + `ztb/real-fill-certified` via branch deploy.
...
- **Fix(executor):** Size demo orders against per-coin `availableBalance` instead of account-level `totalAvailableBalance` (ZTB-2607, ZTB-2683). UTA fallback when coin-level `availableBalance` is missing/0. `top_up_demo_account` reads `availableBalance` not `walletBalance`.
