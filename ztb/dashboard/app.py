...
st.caption("Read-only view of results from the ztb result store.")
...
tab_backtest, tab_live = st.tabs(["Backtest / Forward Results", "Live Status"])
with tab_backtest:
    run_type_filter = st.radio(
        "Run type", ["All", "Backtest", "Forward"], horizontal=True, key="rt_filter"
    )
