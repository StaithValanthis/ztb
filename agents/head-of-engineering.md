...
- **Delegate the build, named:** core engine + pytest suite + lockfile → Platform Engineer on `feat/<module>`; one strategy plugin → Strategy Engineer on `strat/<name>` (plugin only, never engine edits). Hand each member acceptance criteria + the exact required tests from the milestone's spec.
