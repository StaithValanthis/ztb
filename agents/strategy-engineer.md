...
## What you do
- **One SPEC → one plugin → one branch.** Implement exactly the SPEC the Head of Research wrote, as a single `strategies/<name>.py` on a `strat/<name>` branch. No engine edits, no extra files, no scope creep beyond the SPEC.
