...
import { isOpenCodeUnknownSessionError, isOpenCodeTransientUpstreamError, parseOpenCodeJsonl } from "./parse.js";
import { ensureOpenCodeModelConfiguredAndAvailable, parseOpenCodeModelsOutput, requireOpenCodeModelId, } from "./models.js";
...
function readOpenCodeTransientFallbackMode(context) {
    const value = asString(context.opencodeTransientFallbackMode, "").trim();
    switch (value) {
        case "same_model":
        case "fallback_model":
            return value;
...
function openCodeFallbackUsesFallbackModel(mode) {
    return mode === "fallback_model";
}
export async function execute(ctx) {
    ...
    const openCodeTransientFallbackMode = readOpenCodeTransientFallbackMode(context);
    const fallbackModelHint = openCodeFallbackUsesFallbackModel(openCodeTransientFallbackMode)
        ? asString(context.fallbackModel, "").trim() || null
        : null;
    const effectiveModel = fallbackModelHint || model;
    if (fallbackModelHint && fallbackModelHint !== model) {
        onLog("stdout", `[paperclip] OpenCode transient fallback: using model "${fallbackModelHint}" (primary "${model}" is unavailable)\n`);
    }
    ...
    try {
        ...
        const commandNotes = (() => {
            ...
            if (fallbackModelHint) {
                notes.push(`OpenCode transient fallback active: using model "${fallbackModelHint}" (primary "${model}" unavailable).`);
            }
            if (!resolvedInstructionsFilePath)
                return notes;
        ...
        const toResult = (attempt, clearSessionOnMissingSession = false) => {
            ...
            const transientUpstream = (synthesizedExitCode ?? 0) !== 0 &&
                isOpenCodeTransientUpstreamError({
                    stdout: attempt.proc.stdout,
                    stderr: attempt.proc.stderr,
                    errorMessage: fallbackErrorMessage,
                });
            const modelId = effectiveModel || null;
            return {
